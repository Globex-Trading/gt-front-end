...
//         }, 1000);
    ...
    const handleSubmit = async() => {
        if(selectedValue.length > 0) {
            ...
            const selectedPairs = selectedTradingPairs.map(item => ({symbol: item.name, provider: item.provider.slug, id: item._id}));
            setWatchlist([...preWatchList, ...selectedPairs]);
            const symbolIdList = selectedTradingPairs.map(item => item._id);
